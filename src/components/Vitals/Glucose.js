...
const info = {
	title: 'Glucose',
	type: kebabCase('Glucose'),
	inputs: [{ name: 'glucose', title: 'Glucose', weight: 'mg/dL' }],
};

const Glucose = ({ vitals, task }) => {
		...
		try {
			...
			cloneVitals
				...
				.forEach((item, index) => {
					...
					const res = { name: date, item: item.reading.glucose };
					data = [...data, res];
				});
			...
			if (vitals.length > 0) {
				...
				setCurrentVitals({
					...lastReading,
					_reading: lastReading.reading.glucose,
				});
			}
			setData(data);
		} catch (e) {}
