...
const info = {
	...
	type: kebabCase('Head Circumference'),
	inputs: [
		{ name: 'head_circumference', title: 'Head Circumference', weight: 'cm' },
	],
};

const HeadCircumference = ({ vitals, task }) => {
	...
	useEffect(() => {
		try {
			...
			if (vitals.length > 0) {
				...
				setCurrentVitals({
					...lastReading,
					_reading: lastReading.reading.head_circumference,
				});
			}
			setData(data);
		} catch (e) {}
...
export default connect(mapStateToProps)(HeadCircumference);
