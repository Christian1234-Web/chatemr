		...
		try {
			...
			cloneVitals
				...
				.forEach((item, index) => {
					...
					const res = { name: date, item: item.reading.pcv };
					data = [...data, res];
				});
			...
			if (vitals.length > 0) {
				...
				setCurrentVitals({
					...lastReading,
					_reading: lastReading.reading.pcv,
				});
			}
			setData(data);
		} catch (e) {}
			...
			<div className="col-8">
				<div className="el-chart-w">
					...
					>
						...
						<XAxis dataKey="name" />
						<YAxis
							label={{ value: `PCV (${unit})`, angle: -90, position: 'left' }}
						/>
						<Tooltip />
						<Legend />
					</LineChart>
...
export default connect(mapStateToProps)(PCV);
