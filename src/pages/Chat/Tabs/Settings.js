		...
		<React.Fragment>
			<div>
				...
				<div className="text-center border-bottom p-4">
					<div className="mb-4 profile-user d-flex justify-content-center">
					...
					>
						<DropdownToggle tag="a" className="text-muted pb-1 d-block">
							{socket.connected === true ? t('Available') : t('Busy')}
							<i className="mdi mdi-chevron-down"></i>
						</DropdownToggle>

						<DropdownMenu>
							<DropdownItem onClick={connect}>{t('Available')}</DropdownItem>
							<DropdownItem onClick={disconnect}>{t('Busy')}</DropdownItem>
						</DropdownMenu>
					</Dropdown>
				</div>
				{/* End profile user */}
						...
						<Card className="shadow-none border mb-2">
							...
							>
								...
								<div>
									...
									<h5 className="font-size-14">{t(staffname(loggedUser))}</h5>
								</div>

								<div className="mt-4">
									<p className="text-muted mb-1">{t('Email')}</p>
									<h5 className="font-size-14">{t(loggedUser?.email)}</h5>
								</div>
