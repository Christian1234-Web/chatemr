...
//actions
import { createGroup, activeGroup } from '../../../reducers/actions';
...
class Groups extends Component {
	...
	async addRoom() {
		try {
			if (this.state.selectedContact.length >= 2) {
				...
				const url_contact = `messages/chat/staffs/add-to-room`;
	...
	async fetchGroups() {
		try {
			...
			if (rs.success === true) {
				this.setState({ chatGroup: rs.result });
			}
		} catch (err) {
			console.log(err);
		}
	...
	render() {
		const { t } = this.props;
		const { show, chatGroup } = this.state;
		const isNew = true;
		const unRead = 10;
		return (
			<React.Fragment>
...
export default connect(mapStateToProps, { createGroup, activeGroup })(
